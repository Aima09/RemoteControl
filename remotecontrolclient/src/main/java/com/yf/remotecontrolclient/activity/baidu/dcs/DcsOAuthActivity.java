 ...
 */
public class DcsOAuthActivity extends DcsBaseActivity implements View.OnClickListener {
    // 需要开发者自己申请client_id
    // client_id，就是oauth的client_id
    private static final String CLIENT_ID = "75sYsodolPzbwISTo8r7e7yWjLE2ERyC";
    // 是否每次授权都强制登陆
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_bd_dcs_oauth);
        initView();
        setOnClickListener();
    }
        ...
        switch (view.getId()) {
            case R.id.btn_login:
                ...
                if (!TextUtils.isEmpty(clientId) && !TextUtils.isEmpty(clientId)) {
                    baiduOauthImplicitGrant = new BaiduOauthImplicitGrant(clientId, DcsOAuthActivity.this.getApplication());
                    baiduOauthImplicitGrant.authorize(DcsOAuthActivity.this, isForceLogin, isConfirmLogin, new BaiduDialog
                            .BaiduDialogListener() {
                        @Override
                        public void onComplete(Bundle values) {
                            Toast.makeText(DcsOAuthActivity.this.getApplicationContext(),
                        ...
                        public void onError(BaiduDialogError e) {
                            if (null != e) {
                                String toastString = TextUtils.isEmpty(e.getMessage())
                                        ? DcsOAuthActivity.this.getResources()
                                        .getString(R.string.err_net_msg) : e.getMessage();
                                Toast.makeText(DcsOAuthActivity.this.getApplicationContext(), toastString,
                                        Toast.LENGTH_SHORT).show();
                            }
                        }
    ...
    private void startMainActivity() {
        Intent intent = new Intent(DcsOAuthActivity.this, DcsMainActivity.class);
