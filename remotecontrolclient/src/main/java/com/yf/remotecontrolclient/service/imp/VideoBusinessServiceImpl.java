...
public class VideoBusinessServiceImpl implements VideoBusinessService {
    ...
    @Override public void sendBsVideoFile(Media media) {
        try {
            Pmmedia pmmedia = new Pmmedia();
            pmmedia.setCmd("BsuVideommedia");
            ...
            pmmedia.setPath("http://" + IpUtil.getLocalIpAddress(App.getAppContext()) + ":8089?method=afda&path=" + URLEncoder.encode(media.getPath(), "UTF-8"));
            MinaMessageManager.getInstance()
